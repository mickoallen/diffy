...
pub async fn get_diff_summary(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        Ok(DiffSummary { files, total_additions, total_deletions })
...
#[tauri::command]
pub async fn get_workdir_summary(path: String) -> Result<DiffSummary, String> {
    tokio::task::spawn_blocking(move || {
        let repository = git_repo::open_repo(&path)?;
...
#[tauri::command]
pub async fn get_workdir_file_diff(path: String, file_path: String) -> Result<FileDiff, String> {
    tokio::task::spawn_blocking(move || {
        let repository = git_repo::open_repo(&path)?;
...
#[tauri::command]
pub async fn get_branch_to_workdir_summary(path: String, base_ref: String) -> Result<DiffSummary, String> {
    tokio::task::spawn_blocking(move || {
        let repository = git_repo::open_repo(&path)?;
...
pub async fn get_local_vs_remote(path: String) -> Result<(DiffSummary, String, String), String> {
    tokio::task::spawn_blocking(move || {
        ...
        let total_additions = files.iter().map(|f| f.additions).sum();
        let total_deletions = files.iter().map(|f| f.deletions).sum();
        Ok((DiffSummary { files, total_additions, total_deletions }, upstream, branch))
    }).await.map_err(|e| e.to_string())?
}
